Feature: Create job entry in the application
  ...
  Scenario Outline: Create Job entry in the application- Read jsonPayload - <jobid>
    ...
    * eval if (response.status == 400) karate.fail("Status is equal to 400")
  ...
  Scenario: call other scenario
  * def result = call read("createjobentry_call_other_scenario.feature@tag1")